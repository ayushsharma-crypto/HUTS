...
parser.add_argument(
	'--save_trans', type=str, required = False,
...
parser.add_argument(
	'--save_ortho', type=str, required = False,
	help='Saves orthographic img'
)



parser.add_argument(
	'--save_persp', type=str, required = False,
	help='Saves perspective img'
)


parser.add_argument(
	'--save_matches', type=str, required = False,
	help='Saves match quality'
)


parser.add_argument(
	'--save_kp', type=str, required = False,
	help='Saves match correspondences'
)


parser.add_argument(
	'--save_desp', type=str, required = False,
	help='Saves match correspondences'
)
...
if __name__ == "__main__":
	...
	if args.model_rord:
		srcPts, trgPts, matchImg, matchImgOrtho, match_quality = getPerspKeypoints(args.rgb1, args.rgb2, srcH, trgH, model2, device)
		if args.save_matches:
			F = open(args.save_matches, 'a')
			F.write(
				"rgb1 "+ match_quality["rgb1"]+" "+
				"rgb2 "+ match_quality["rgb2"]+" "+
				str(match_quality["inliers"])+"\n"
			)
			F.close()
			print("Match quality saved.")
		if args.save_desp:
			arr = np.array([
				match_quality["inlier_keypoints_left"],
				match_quality["inlier_keypoints_right"],
				match_quality["ortho_desp1"],
				match_quality["ortho_desp2"]
			],dtype=object)
			np.save(args.save_desp, arr)
			print("Orthographic keypoints and Descriptors saved.")

		if args.save_kp:
			match = [match_quality["inlier_keypoints_left"], match_quality["inlier_keypoints_right"]]
			match = np.array(match)
			print("match = ",match)
			rgb_1 = re.search('\d+', match_quality["rgb1"].replace(' ',''))[0]
			rgb_2 = re.search('\d+', match_quality["rgb2"].replace(' ',''))[0]
			np.save(os.path.join(args.save_kp, rgb_1+"-"+rgb_2), match)
	...
	if args.save_persp:
		cv2.imwrite(args.save_persp, matchImg)
		print("Perspective Img saved.")

	if args.save_ortho:
		cv2.imwrite(args.save_ortho, matchImgOrtho)
		print("Orthographic Img saved.")
