...
parser.add_argument('--netvlad_distances', type=str, default='', help='Path to NetVLAD Distances')
...
if __name__ == "__main__":
    ...
    netvlad_distances = np.load(opt.netvlad_distances)
    # print("netvlad_candidates= ",netvlad_candidates)
    # print("netvlad_distances= ",netvlad_distances)
    ...
    for i in range(netvlad_candidates.shape[0]):
      for j in range(netvlad_candidates.shape[1]):
        ...
        candidate_dist_list.append([q_lst[i], db_lst[netvlad_candidates[i,j]], netvlad_distances[i,j]])
    ...
    df.to_csv(os.path.join(opt.save_path, opt.dataset+'_netvlad_candidate_distance_list.txt'), header=None, index=None, sep=' ', mode='a')