...
def extract_features(db, net, trfs, pooling='mean', gemp=3, detailed=False, whiten=None,
    ...
    bdescs = []
    qdescs = []
    # bdescs_before_pooling = []
    # qdescs_before_pooling = []

    trfs_list = [trfs] if isinstance(trfs, str) else trfs
    print("trfs_list = ",trfs_list)
    for trfs in trfs_list:
        ...
        img_feats, _ = test.extract_image_features(db, trfs, net, desc="DB", **kw)
        bdescs.append(img_feats)
        # bdescs_before_pooling.append(before_pooling_img)
        print("bdesc = ",len(bdescs), bdescs[0].shape)
        # extract query feats
        if query_db is not None:
            if db is query_db:
                qdescs.append(bdescs[-1])
                # qdescs_before_pooling.append(bdescs_before_pooling[-1])
            else:
                img_feats, _ = test.extract_image_features(query_db, trfs, net, desc="query", **kw)
                qdescs.append(img_feats)
                # qdescs_before_pooling.append(before_pooling_img)
    ...
    if query_db is db or query_db is None:
        ...
        # A = np.array([t.numpy() for t in bdescs_before_pooling[0]])
        # np.save(output+"_before_pooling",A)
    else:
...
def load_model(path, iscuda):
    ...
    print("checkpoint = ", checkpoint["model_options"])
