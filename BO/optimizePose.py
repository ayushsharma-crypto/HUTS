...
if __name__ == '__main__':
	...
	X_gt, Y_gt, THETA_gt = readG2o(argv[3])
	# draw(X, Y, THETA)

	trg,src, trans = readLC(argv[2])
	# src, trg, trans = src[-1], trg[-1], trans[-1]
	# src, trg, trans = [], [], []
	...
	optimize(argv[4])
	(xOpt, yOpt, tOpt) = readG2o(argv[4])
	drawLC(xOpt, yOpt, tOpt, src, trg)
	drawThree(X_gt,Y_gt,X,Y,xOpt,yOpt)